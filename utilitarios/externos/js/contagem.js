$(function () {
    var endDate = "June 17, 2016 20:00:00";
    ...
    $('.countdown.styled').countdown({
        ...
        render: function (data) {
            $(this.el).html("<div>" + this.leadingZeros(data.days, 3) + " <span>dias</span></div><div>" + this.leadingZeros(data.hours, 2) + " <span>horas</span></div><div>" + this.leadingZeros(data.min, 2) + " <span>minutos</span></div><div>" + this.leadingZeros(data.sec, 2) + " <span>segundos</span></div>");
        }
    });
    
    $('.countdown.styled_rodape').countdown({
        date: endDate,
        render: function (data) {
            $(this.el).html("<div>" + this.leadingZeros(data.days, 3) + " <span>dias</span></div><div>" + this.leadingZeros(data.hours, 2) + " <span>horas</span></div><div>" + this.leadingZeros(data.min, 2) + " <span>minutos</span></div><div>" + this.leadingZeros(data.sec, 2) + " <span>segundos</span></div>");
        }
